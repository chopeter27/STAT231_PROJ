...
tidy_combined_bin <- tidy_combined_a %>%
  ...
  group_by(uuid) %>%
  summarise(n = n(), avgscore = sum(score) / n, type = mean(binary_type)) %>%
  mutate(articlesent = ifelse(avgscore < 0, "negative", "positive"))
tidy_combined_bin

n <- nrow(tidy_combined_bin)
train_id <- sample(1:n, size = round(n*0.8))
train <- tidy_combined_bin[train_id,]
test <- tidy_combined_bin[-train_id,]

tree <- rpart(type ~ avgscore, data = train)
plot(as.party(tree))
tree
...
roc_obj <- roc(test$type, test$prediction)
auc(roc_obj)
plot(roc_obj)
