...
```{r}
tokens <- data_frame(text = fake$text) %>% unnest_tokens(word, text) #would be helpful to find a way to add author column and type column to tokens
tokens %>%
...
Project goals: --> I dont think we need this part anymore as per our new goals but I kept it in here just in case --> problems in line 53 and 58
```{r}
#remove stopwords
fake_edit <- fake
